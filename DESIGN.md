...
> S2EE is a precise event operating surface: institutional, indexed, Satoshi-led, navy/yellow, and public-facing without becoming marketing.
...
| [`115:68`](https://www.figma.com/design/SSTjQLy3S5IVplllh3YMXU/Visual-identity?node-id=115-68&m=dev)   | Current VI landing board   | Canonical visual identity for the public landing: S2EE 17 mark, wave field, ETIC presence, section rhythm, and mood. |
| [`115:69`](https://www.figma.com/design/SSTjQLy3S5IVplllh3YMXU/Visual-identity?node-id=115-69&m=dev)   | Current VI UI board        | Canonical frontend translation reference: public sections, featured company cards, plan embedding, spacing, and states. |
...
When the Figma nodes disagree, resolve them in this order:

1. Nodes `115:68` and `115:69` for the current visual identity and frontend direction.
2. Node `115:70` for final broad visual QA.
3. Earlier system nodes such as `93:*`, `101:*`, and `113:*` for reusable tokens, controls, surfaces, workspace shells, and state behavior.

The newer VI nodes do not turn the product into a generic marketing site. They clarify how the public S2EE identity should look while the authenticated areas remain operational, dense, and task-first.
...
- Satoshi-led
- operational
- deep navy with pale gray and yellow identity accents
- information-first
...
| -------------- | ------------------------------ | -------------------- | -------------------------------------------------- |
| Canvas         | `--s2ee-canvas`                | `#0B132B`            | Main app/page background.                          |
| Canvas wash    | `--s2ee-canvas-glow`           | `#101A36`            | Subtle grid wash or secondary background.          |
| Surface        | `--s2ee-surface`               | `#101A36`            | Forms, inspectors, active content planes.          |
| Surface soft   | `--s2ee-surface-soft`          | `#132143`            | Headers, side rails, inactive sheets, legends.     |
| Primary text   | `--foreground` / identity gray | `#EAEAEA`            | Primary text and strong labels.                    |
| Secondary text | `--s2ee-soft-foreground`       | `#EAEAEA`            | Supporting hierarchy.                              |
| Muted text     | `--s2ee-muted-foreground`      | mixed from `#EAEAEA` | Metadata and helper text.                          |
| Border         | `--s2ee-border`                | mixed from `#EAEAEA` | Default dividers and panels.                       |
| Strong border  | `--s2ee-border-strong`         | mixed from `#EAEAEA` | Active frames, selected rows, stronger separators. |
| Accent         | `--color-primary`              | `#F4E87C`            | Primary action, active route, selected pin, focus. |
...
- Do not introduce purple, beige, cream, warm brown, generic blue accents, or large gradients.
...
Use Satoshi as the product and identity family.
...
Public identity surfaces should lean heavy Satoshi weights. Dense workspace surfaces may use medium Satoshi for readability, with tracked Satoshi labels reserved for codes, metadata, and command elements.

### Type Behavior

- Brand marks and edition labels: use the Figma mark asset first; if live text is required, use heavy Satoshi with positive tracking.
- Route labels: uppercase, Satoshi, tracked.
- Room codes and status tokens: compact, tracked, machine-readable.
...
- Light plane: active forms, active table area, featured company cards, and map inspector content.
...
### Public VI Components

Use nodes `115:68` and `115:69`.

Rules:

- The public landing may use a stronger visual identity layer than workspace screens.
- The wave or motion-line field is a real identity asset, not generic decoration.
- ETIC Club should appear as an organizer signal, especially on the public landing.
- Featured company cards belong to the public landing only when driven by admin-published data.
- The salon plan can be embedded on the landing, but the dedicated `/map` route remains the full public navigation board.
- Keep the yellow accent as a command and emphasis color. Do not use it as a large background plane.
- Light gray company/card surfaces should sit on the navy canvas and keep enough contrast for headings, body text, and offer counts.

## Page Directives

### Public Home `/`

Use nodes `115:68`, `115:69`, `93:727`, `93:454`, `93:485`, and `93:535`.

The public home is the S2EE 17 landing surface. It should feel event-led and artistic while staying useful enough to route visitors into the platform.

It should provide:

- S2EE identity and 17e edition.
- One sentence explaining the event: `S2EE` means `Salon de l'emploi de l'ESI`.
- ETIC Club as the visible organizer.
- Primary route into sign-in.
- Secondary route to the public map or plan section.
- Featured companies published by the admin.
- Embedded salon plan as a later section, not the main story.
- Event facts limited to real public identity facts: edition, date, institution, and organizer.

Composition:

- Desktop: dark identity-led canvas with the Figma `S2EE 17` mark, wave/motion-line field, ETIC organizer plane, and strong section dividers.
- Mobile: identity first, concise event sentence, route actions reachable without scrolling too far, facts and sections below.
- Sections: hero, event atmosphere, featured companies, salon plan.
- Use real event facts only.

Assets:

- Use `/brand/s2ee-17-mark.svg` for the primary identity mark when available.
- Use `/brand/s2ee-wave.svg` for the wave or motion field when a visual identity background is needed.
- Use `/etic.svg` for ETIC Club organizer presence.
- Do not redraw the S2EE mark as loose live text for major identity moments.
- When `S2EE`, `S2EE 17`, or compact wordmarks must be live text, use positive tracking through `.s2ee-small-wordmark`.
...
- account creation facts in the hero
- access-model explainers in the hero
- companies derived from map placements instead of admin-published featured company data
...
Before shipping a broad UI change, check it against nodes `115:68`, `115:69`, and `115:70`, then use this list:
...
- No gradient-only hero backgrounds. Public identity atmosphere must come from the VI assets or real media, not CSS decoration alone.
- No fake metrics.
...
> Build S2EE as a precise employment-fair operating surface. Use the Figma Visual identity file as the source of truth, especially nodes `93:454`, `93:631`, `93:521`, `93:535`, `93:650`, `93:677`, `93:727`, `93:737`, `101:814`, `101:836`, `115:68`, and `115:69`. The app should be deep navy, pale gray, yellow-accented, institutional, Satoshi-led, and information-first. Build structure from split planes, command bars, rails, inspector panes, ruled lists, tables, and map canvases. Avoid marketing cards, fake metrics, decorative gradients, warm tones, and generic SaaS styling. Every element must represent a real event object, route, state, or command.
